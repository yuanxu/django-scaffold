...
formula.dialog = (function () {
    ...
    function mathHtml(obj) {
        ...
        for (var i = 0; i < obj.count; i++) {
            html += "<li onclick=\"formula.dialog.insertFormula('" + formulas[obj.groupid][i] + "')\" style=\"background-position:-" + (obj.x + Math.floor(i % 8) * slidLen) + "px -" + (obj.y + Math.floor(i / 8) * slidLen) + "px;\"></li>";
        }
        html += "</div>";
        if (obj.count > cols * 2) {
    ...
    this.insertFormula = function (q) {
        $('#'+this._editorInputBox).mathquill("write", q.replace("{/}", "\\")).focus();
    };

    this._editorInputBox = '';

    /**
     * 初始化
     * @param editorInputBoxId 编辑框id
     */
    this.initDialog = function(editorInputBoxId) {

        this._editorInputBox = editorInputBoxId;
